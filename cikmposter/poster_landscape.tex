...
We obtained sports data from the 2011 Nationala Football League (NFL) Season, POS data from CoNLL-2000 and moview review data from Cornell.
\begin{center}
\begin{tabular}{|l|r|r|}
\hline
...
\textit{SELECT term, pos}\\
\textit{FROM postag(<text>)};

								Here is a function for named-entity recognition.\\
\textit{SELECT term, tag}\\
\textit{FROM ne\_chunk(<text>);}
...
\fbox{%
        \parbox{0.95\linewidth}{%
\textcolor{blue}{\bf $\bf Q_2$:Feature generation for training data}\\
